...
const { success, badRequest } = require('../util/movieApiResponseHandler');
  ...
  try {
    ...
    const order =
      req.query.order && req.query.order.toLowerCase() === 'desc'
        ? 'DESC'
        : 'ASC';
    const { page: currentPage, data } = await movieService.listMoviesByYear(
      year,
      page,
      order
    );
...
exports.moviesByGenre = async (req, res, next) => {
  try {
    ...
    const page = parseInt(req.query.page, 10) || 1;
    const { page: currentPage, data } = await movieService.listMoviesByGenre(
      genre,
      page
    );
    return success(res, data, { page: currentPage });
