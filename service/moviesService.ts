...
const MovieApiError = require('../util/movieError');
...
async function getMovieDetails(imdbId: string): Promise<MovieDetails> {
  ...
  const avg =
    ratingRow && ratingRow.avgRating ? parseFloat(ratingRow.avgRating) : 0;
  return {
    imdbId: movie.imdbId,
    title: movie.title,
    ...
    productionCompanies: JSON.parse(movie.productionCompanies).map(
      (pc) => pc.name
    ),
  };
}
